...
## 9. 安全考试、预约、报告、推荐与 AI 模块整合

成员 C 子系统已经纳入统一后端工程，使用同一套登录、角色、权限、异常和返回结构：

- `/api/questions`：题库分页、详情、新增、批量导入、修改、删除。
- `/api/exams/papers`：试卷分页、详情、创建、编辑、发布/关闭、题目维护。
- `/api/exams`：学生可参加考试、开始考试、提交答案、考试记录、错题本。
- `/api/exams/statistics`：考试总览、分数分布、题目分析、知识点薄弱分析、短答题批改。
- `/api/reservations`：实验时间段管理、可预约时间段、预约申请、取消、审核。
- `/api/reports`：实验报告草稿、提交、详情、待批改列表、评分、退回。
- `/api/recommendations`：基于实验、错题、学习记录和资源热度生成推荐资源。
- `/api/ai`：本地知识库辅助问答、错题解释、报告建议和人工修订记录。
...
整合后的权限边界由 `Authorization` Token、`UserContext` 和 `@RequirePermission` 共同完成；学生自助数据在服务层继续校验记录归属，避免通过 ID 访问他人考试、预约、报告或 AI 记录。
