            ...
            <div class="step-body">
              ...
              <div v-if="selectedStep.mediaUrl" class="step-media-preview">
                ...
                <iframe
                  ...
                  title="步骤视频或资料"
                />
                <a v-else :href="selectedStep.mediaUrl" target="_blank" rel="noreferrer">打开步骤视频或资料链接</a>
              </div>
              <aside>
                <b>风险提示</b>
        ...
        <nav class="course-catalog">
          <section v-for="group in filteredCourseCatalog" :key="group.experimentId" class="catalog-group">
            ...
            <button
              ...
              type="button"
              class="catalog-step"
              :class="{ active: node.experimentId === experimentId && node.target === activeTarget }"
              @click="openStep(node)"
            >
...
async function loadPage() {
  ...
  try {
    ...
    await loadAllExperimentDetails()
...
.catalog-step span { width: 22px; height: 22px; border-radius: 50%; background: #4cc96f; color: #fff; display: inline-flex; align-items: center; justify-content: center; }
